...
class TestController extends Controller
{
    ...
    public function getCostCenterWithFundAllocations(Request $request)
    {
        ...
        $cost_centers = CostCenter::whereIn('id', $cost_center_ids)->get()->map(function ($cost_center) use ($fund_allocations) {
            $cost_center->fund_allocations = collect($fund_allocations)->where('cost_center_id', $cost_center->id)->toArray();
            $cost_center->total_initial_amount = collect($cost_center->fund_allocations)->sum('initial_amount');
            return $cost_center;
        });
        return $cost_centers;
    }
    public function getTotalProgrammed($is_supplemental, $fund_cluster_w_f_p_s_id, $selectedType, $m_f_o_s_id)
    {
        return WfpDetail::whereHas('wfp', function ($query) use ($selectedType, $is_supplemental, $fund_cluster_w_f_p_s_id, $m_f_o_s_id) {
