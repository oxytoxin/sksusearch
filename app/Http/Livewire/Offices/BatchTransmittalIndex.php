...
class BatchTransmittalIndex extends Component implements HasTable
{
    ...
    protected function getTableQuery()
    {
        ...
        if ($this->tab === 'incoming') {
            return BatchTransmittal::query()
                ->where(function ($q) use ($officeGroupId) {
                    $q->whereHas('items.disbursement_voucher', function ($q) use ($officeGroupId) {
                        $q->whereRelation('current_step', 'office_group_id', $officeGroupId);
                    })->orWhereHas('items.liquidation_report', function ($q) use ($officeGroupId) {
                        $q->whereRelation('current_step', 'office_group_id', $officeGroupId);
                    });
                })
    ...
    protected function getTableColumns(): array
    {
        return [
            ...
            TextColumn::make('document_type')->label('Type')
                ->formatStateUsing(fn ($state) => $state === 'liquidation_report' ? 'Liquidation Reports' : 'Disbursement Vouchers'),
        ...
        return [
            Action::make('receive_batch')
                ...
                ->action(function (BatchTransmittal $record) {
                    ...
                    DB::transaction(function () use ($record, &$received, &$skipped) {
                        if ($record->document_type === 'liquidation_report') {
                            foreach ($record->items()->with('liquidation_report.current_step')->get() as $item) {
                                $lr = $item->liquidation_report;
                                if ($lr && $lr->current_step->process === 'Forwarded to') {
                                    $lr->update(['current_step_id' => $lr->current_step->next_step->id]);
                                    $lr->refresh();
                                    $lr->activity_logs()->create([
                                        'description' => $lr->current_step->process.' '.auth()->user()->employee_information->full_name,
                                    ]);
                                    $received++;
                                } else {
                                    $skipped++;
                            ...
                            foreach ($record->items()->with('disbursement_voucher.current_step')->get() as $item) {
                                ...
                                if ($dv && $dv->current_step->process === 'Forwarded to') {
                    ...
                    $label = $record->document_type === 'liquidation_report' ? 'LR' : 'DV';
                    $msg = "Batch received. {$received} {$label}(s) received.";
                    if ($skipped > 0) {
                        $msg .= " {$skipped} {$label}(s) skipped (already received or returned).";
                    }
