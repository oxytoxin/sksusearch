    ...
    protected function getTableActions(): array
    {
        return [
           ...
           ViewAction::make('FMD')
            ...
            //    ->modalWidth('4xl')
               ->url(fn ($record) => route('formal-management-demand', ['record' => $record->caReminderStep->disbursementVoucher]))
               ->button()
               ->color('primary')
           ...
           ViewAction::make('FMR')
            ...
            //    ->modalWidth('4xl')
               ->url(fn ($record) => route('formal-management-reminder', ['record' => $record->caReminderStep->disbursementVoucher]))
               ->button()
               ->color('primary')
               ->icon('heroicon-o-document-text')
               ->tooltip('View FMR')
           ...
           ViewAction::make('FD')
            ...
            //    ->modalWidth('4xl')
                ->url(fn ($record) => route('endorsement-for-fd', ['record' => $record->caReminderStep->disbursementVoucher]))
               ->button()
               ->color('primary')
               ->icon('heroicon-o-document-text')
               ->tooltip('View FD')
           ...
           ViewAction::make('SCO')
            ...
            //    ->modalWidth('4xl')
               ->url(fn ($record) => route('show-cause-order', ['record' => $record->caReminderStep->disbursementVoucher]))
               ->button()
               ->color('primary')
               ->icon('heroicon-o-document-text')
               ->tooltip('View SCO')
