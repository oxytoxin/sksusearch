    ...
    class LiquidationReportsCreate extends Component implements HasForms
    {
                            ...
                            Grid::make(2)->schema([
                                Select::make('disbursement_voucher_id')
                                    ...
                                    ->afterStateUpdated(function ($set, $state) {
                                        $this->disbursement_voucher = DisbursementVoucher::with('travel_order')->withSum('disbursement_voucher_particulars as total_amount',
                                            'final_amount')->find($state);
        ...
        public function mount()
        {
            ...
            if (request('disbursement_voucher')) {
                $dv = DisbursementVoucher::with('travel_order')->withSum('disbursement_voucher_particulars as total_amount',
                    'final_amount')->whereRelation('voucher_subtype', 'voucher_type_id',
                    1)->findOrFail(request('disbursement_voucher'));
                ...
                if ($dv->travel_order?->travel_order_type_id == TravelOrderType::OFFICIAL_BUSINESS) {
                    $this->generateItineraryEntries($dv->travel_order->itineraries()->firstWhere('user_id',
                        auth()->id()));
                }
            }
        }
        ...
        public function save()
        {
            ...
            if ($this->disbursement_voucher->travel_order?->travel_order_type_id == TravelOrderType::OFFICIAL_BUSINESS) {
                ...
                if (isset($this->data['condition']) && $this->data['condition'] == 2) {
                    $refund_particular = collect($this->data['refund_particulars'])->first();
                    $or_number = $refund_particular['or_number'] ?? null;
                    $or_date = $refund_particular ? date_format(date_create($refund_particular['date']),
                        'm/d/Y') : null;
                    $or_amount = $refund_particular['amount'] ?? null;
        ...
        private function itinerarySection(): array
        {
        ...
        private function generateItineraryEntries($old_itinerary): void
        {
            $entries = [];
