    ...
    class CreateWFP extends Component implements Forms\Contracts\HasForms
    {
        ...
        public function mount($record, $wfpType, $isEdit, $isSupplemental)
        {
            ...
            if ($isEdit == 1) {
                        ...
                        $query->where('wpf_type_id', $wfpType)
                            ->when(!is_null($this->supplementalQuarterId), function ($query) {
                                if ($this->supplementalQuarterId == 1) {
                                    $query->where('supplemental_quarter_id',
                                        $this->supplementalQuarterId)->orWhere('supplemental_quarter_id', null);
                                } else {
                                    $query->where('supplemental_quarter_id', '<=',
                                        $this->supplementalQuarterId)->orWhere('supplemental_quarter_id', null);
                                }
                ...
                $this->record = CostCenter::with([
                    'wfp' => function ($query) use ($wfpType) {
                        ...
                        $query->where('wpf_type_id', $wfpType)->when(!is_null($this->supplementalQuarterId),
                            function ($query) {
                                $query->where('is_supplemental', 0)->orWhere(function ($query) {
                                    $query->where('supplemental_quarter_id', '!=',
                                        null)->where('supplemental_quarter_id', '<=', $this->supplementalQuarterId);
                                });
                            })->with('categoryGroup');
                    }
            ...
            if ($isSupplemental) {
                ...
                $this->wfp_fund = $this->record->fundAllocations->where('wpf_type_id',
                    $wfpType)->where('supplemental_quarter_id', $this->supplementalQuarterId)->first()->fundClusterWFP;
                $this->fund_allocations = $this->record->fundAllocations->where('wpf_type_id',
                    $wfpType)->where('supplemental_quarter_id', $this->supplementalQuarterId);
            } else {
                $this->wfp_type = $this->record->fundAllocations->where('wpf_type_id', $wfpType)->first()->wpfType;
                $this->wfp_fund = $this->record->fundAllocations->where('wpf_type_id',
                    $wfpType)->first()->fundClusterWFP;
                $this->fund_allocations = $this->record->fundAllocations->where('wpf_type_id', $wfpType);
            }

            $this->costCenter = $this->record->where('office_id',
                auth()->user()->employee_information->office_id)->first();
            ...
            if (in_array($this->wfp_fund->id, [1, 3, 9])) {
                ...
                if ($isSupplemental) {
                    if ($this->record->fundAllocations->where('wpf_type_id', $wfpType)->where('supplemental_quarter_id',
                        ...
                        // HERE DRAFT
                        $draft_amounts = $this->record->fundAllocations->where('wpf_type_id',
                            $wfpType)->where('supplemental_quarter_id',
                            $this->supplementalQuarterId)->first()->fundDrafts->first()->draft_items()->get();
                        if ($draft_amounts) {
                        ...
                        $this->current_balance = $costCenterFundAllocations
                            ...
                            ->map(function ($allocation) use (
                                ...
                                if ($allocation->supplemental_quarter_id !== $this->supplementalQuarterId && !empty($all_current_allocation->where('category_group_id',
                                        $allocation->category_group_id)->first())) {
                                    return null;
                                }
                                if ($allocation->supplemental_quarter_id === $this->supplementalQuarterId) {
                                    return [
                        ...
                        $this->programmed_non_supplemental = array_sum(array_diff_key($allocation_non_supplemental,
                    ...
                    if ($this->record->fundAllocations->where('wpf_type_id',
                        $wfpType)->first()->fundDrafts()->first()?->draft_amounts()->exists()) {
                        $initial_amount = $this->record->fundAllocations->where('wpf_type_id', $wfpType);
                        $draft_amounts = $this->record->fundAllocations->where('wpf_type_id',
                            $wfpType)->first()->fundDrafts->first()->draft_items;
                        $this->current_balance = $this->record->fundAllocations->where('wpf_type_id', $wfpType)
                            ...
                            ->map(function ($allocation) use ($initial_amount, $draft_amounts) {
                                return [
                                    'category_group_id' => $allocation->category_group_id,
                                    'category_group' => $allocation->categoryGroup->name,
                                    'initial_amount' => $initial_amount->where('category_group_id',
                                            $allocation->category_group_id)->first()->initial_amount ?? 0,
                                    'current_total' => $draft_amounts->where('title_group',
                                            $allocation->category_group_id)->sum('estimated_budget') ?? 0,
                                    'balance' => $initial_amount->where('category_group_id',
                                            $allocation->category_group_id)->first()->initial_amount ?? 0 - $draft_amounts->where('title_group',
                                            $allocation->category_group_id)->sum('estimated_budget') ?? 0,
                                ];
                            })
                            ->toArray();
                    ...
                    if ($this->record->fundAllocations->where('wpf_type_id', $wfpType)->where('supplemental_quarter_id',
                        ...
                        if ($isSupplemental) {
                            ...
                            if (count($this->record->wfp) > 0) {
                                ...
                                $fundDraftId = $this->record->fundAllocations->where('wpf_type_id',
                                    $wfpType)->where('supplemental_quarter_id',
                                    $this->supplementalQuarterId)->first()->fundDrafts->first()->id;
                                $fdrafts = FundDraftAmount::where('fund_draft_id',
                                    $fundDraftId)->with(['fundDraft'])->get();
                                ...
                                $this->current_balance = $this->record->fundAllocations->where('wpf_type_id',
                                    $wfpType)->where('supplemental_quarter_id',
                                    $this->supplementalQuarterId)->first()->fundDrafts->first()->draft_amounts->map(function (
                                    $allocation
                                ) {
                                    return [
                                        'category_group_id' => $allocation->category_group_id,
                            ...
                            $this->current_balance = $this->record->fundAllocations->where('wpf_type_id',
                                ...
                                0)->first()->fundDrafts->first()->draft_amounts->map(function ($allocation) {
                                return [
                                    'category_group_id' => $allocation->category_group_id,
                                    'category_group' => $allocation->category_group,
                                    'initial_amount' => $allocation->initial_amount,
                        ...
                        $initial = $this->record->fundAllocations->where('wpf_type_id',
                            $wfpType)->first()->initial_amount;
                        $this->wfp_balance = $initial - array_sum($programmed);

                        $balance = array_sum($prev_allocations) - array_sum($programmed);
                        $this->current_balance = $this->record->fundAllocations->where('wpf_type_id',
                            $wfpType)->where('supplemental_quarter_id', $this->supplementalQuarterId)->map(function (
                            $allocation
                        ) use ($balance) {
                            return [
                                'category_group_id' => $allocation->category_group_id,
            ...
            if ($isSupplemental) {
                ...
                if ($this->record->fundAllocations->where('wpf_type_id',
                    $this->wfp_param)->first()->fundDrafts()->exists()) {
                    //1
                    $this->record->fundAllocations->where('wpf_type_id',
        ...
        public function updatedSuppliesQuantity()
        {
            ...
            $budget_category_id = BudgetCategory::where('id',
                $supply->categoryItems()->first()->budget_category_id)->first()->id;
            switch ($budget_category_id) {
                ...
                case 2:
                    ...
                    $this->supplies_estimated_budget = number_format($this->supplies_total_quantity * $cost_per_unit,
                        2);
                    break;
                case 3:
                    $this->calculateTrainingTotalQuantity();
        ...
        public function addSupplies()
        {
            ...
            if ($this->is_supplemental) {
                ...
                if ($this->record->fundAllocations->where('wpf_type_id',
                    $this->wfp_param)->where('supplemental_quarter_id',
                    $this->supplementalQuarterId)->first()->fundDrafts()->exists()) {
                    $draft_id = $this->record->fundAllocations->where('wpf_type_id',
                        $this->wfp_param)->where('supplemental_quarter_id',
                        $this->supplementalQuarterId)->first()->fundDrafts()->first()->id;
                }
                if ($this->supplies != null) {
                    foreach ($this->supplies as $key => $supply) {
                        if ($supply['particular_id'] == $this->supplies_particular_id && $supply['uom'] == $this->supplies_uom && $supply['remarks'] == $this->supplies_remarks) {
                            ...
                            $draft_items = $this->record->fundAllocations->where('wpf_type_id',
                                $this->wfp_param)->where('supplemental_quarter_id',
                                $this->supplementalQuarterId)->first()->fundDrafts->first()->draft_items->where('particular_id',
                                $this->supplies_particular_id)->where('uom', $this->supplies_uom)->where('remarks',
                                $this->supplies_remarks)->first();
                            ...
                            $existingDraftItem = $this->record->fundAllocations->where('wpf_type_id',
                                $this->wfp_param)->where('supplemental_quarter_id',
                                $this->supplementalQuarterId)->first()->fundDrafts->first()->draft_items->where('particular_id',
                                $this->supplies_particular_id)->where('uom', $this->supplies_uom)->where('remarks',
                                $this->supplies_remarks)->first();
                            if (!$existingDraftItem) {
                                ...
                                FundDraftItem::create(
                                    [
                                        'fund_draft_id' => $this->record->fundAllocations->where('wpf_type_id',
                                            $this->wfp_param)->where('supplemental_quarter_id',
                                            $this->supplementalQuarterId)->first()->fundDrafts->first()->id,
                                        'budget_category_id' => $this->supplies_category_attr->categoryItems->budget_category_id,
                                        'budget_category' => 'Supplies & Semi-Expendables',
                    ...
                    if ($this->record->fundAllocations->where('wpf_type_id',
                        ...
                        $draft = FundDraft::create([
                            'fund_allocation_id' => $this->record->fundAllocations->where('wpf_type_id',
                                $this->wfp_param)->where('supplemental_quarter_id',
                                $this->supplementalQuarterId)->first()->id,
                        ]);
                        ...
                        if ($balance && $balance['category_group_id'] == $categoryGroupId) {
                            ...
                            if ($this->record->fundAllocations->where('wpf_type_id',
                                ...
                                $draft_id = $this->record->fundAllocations->where('wpf_type_id',
                                    $this->wfp_param)->where('supplemental_quarter_id',
                                    $this->supplementalQuarterId)->first()->fundDrafts()->first()->id;
                                $draft_amounts = FundDraftAmount::where('fund_draft_id',
                                    $draft_id)->where('category_group_id', $categoryGroupId)->first();
                        ...
                        foreach ($this->current_balance as $item) {
                            if ($item) {
                                if ($this->record->fundAllocations->where('wpf_type_id',
                                    ...
                                    $draft_amounts = $this->record->fundAllocations->where('wpf_type_id',
                                        $this->wfp_param)->where('supplemental_quarter_id',
                                        $this->supplementalQuarterId)->first()->fundDrafts->first()->draft_amounts->where('category_group_id',
                                        $item['category_group_id'])->first();
                ...
                if ($this->record->fundAllocations->where('wpf_type_id',
                    $this->wfp_param)->first()->fundDrafts()->exists()) {
                    $draft_id = $this->record->fundAllocations->where('wpf_type_id',
                        $this->wfp_param)->first()->fundDrafts()->first()->id;
                }
                if ($this->supplies != null) {
                    foreach ($this->supplies as $key => $supply) {
                        if ($supply['particular_id'] == $this->supplies_particular_id && $supply['uom'] == $this->supplies_uom && $supply['remarks'] == $this->supplies_remarks) {
                            ...
                            $draft_items = $this->record->fundAllocations->where('wpf_type_id',
                                $this->wfp_param)->first()->fundDrafts->first()->draft_items->where('particular_id',
                                $this->supplies_particular_id)->where('uom', $this->supplies_uom)->where('remarks',
                                $this->supplies_remarks)->first();
                            $draft_items->quantity = json_encode($this->supplies[$key]['quantity']);
                            $draft_items->total_quantity = $this->supplies[$key]['total_quantity'];
                ...
                if ($this->wfp_fund->id === 2 || $this->wfp_fund->id === 4 || $this->wfp_fund->id === 5 || $this->wfp_fund->id === 6 || $this->wfp_fund->id === 7) {
                    ...
                    if (!$this->record->fundAllocations->where('wpf_type_id',
                        $this->wfp_param)->first()->fundDrafts()->first()->draft_amounts()->exists()) {
                        foreach ($this->current_balance as $item) {
                            $draft_amounts = FundDraftAmount::create([
                                'fund_draft_id' => $this->record->fundAllocations->where('wpf_type_id',
                                    $this->wfp_param)->first()->fundDrafts->first()->id,
                        ...
                        foreach ($this->current_balance as $item) {
                            ...
                            if ($this->record->fundAllocations->where('wpf_type_id',
                                ...
                                $draft_amounts = $this->record->fundAllocations->where('wpf_type_id',
                                    $this->wfp_param)->first()->fundDrafts->first()->draft_amounts->where('category_group_id',
                                    $item['category_group_id'])->first();
                                $draft_amounts->current_total = $item['current_total'] ?? 0;
                                $draft_amounts->balance = $item['balance'] ?? 0;
                                $draft_amounts->save();
        ...
        public function updatedMooeQuantity()
        {
            ...
            $budget_category_id = BudgetCategory::where('id',
                $mooe->categoryItems()->first()->budget_category_id)->first()->id;
            switch ($budget_category_id) {
                case 1:
                    $this->calculateSuppliesTotalQuantity();
                    $cost_per_unit = $this->mooe_cost_per_unit == null ? 0 : $this->mooe_cost_per_unit;
                ...
                case 3:
                    ...
                    $this->training_estimated_budget = number_format($this->training_total_quantity * $cost_per_unit,
                        2);
                    break;
                case 4:
                    $this->calculateMachineTotalQuantity();
                ...
                case 5:
                    ...
                    $this->building_estimated_budget = number_format($this->building_total_quantity * $cost_per_unit,
                        2);
                    break;
                case 6:
                    $this->calculatePsTotalQuantity();
                    ...
                    foreach ($this->mooe as $key => $mooe) {
                        if ($mooe['particular_id'] == $this->mooe_particular_id && $mooe['uom'] == $this->mooe_uom && $mooe['remarks'] == $this->mooe_remarks) {
                            ...
                            $draft_items = $this->record->fundAllocations->where('wpf_type_id',
                                $this->wfp_param)->where('supplemental_quarter_id',
                                $this->supplementalQuarterId)->first()->fundDrafts->first()->draft_items->where('particular_id',
                                $this->mooe_particular_id)->where('uom', $this->mooe_uom)->where('remarks',
                                $this->mooe_remarks)->first();
        ...
        public function updatedTrainingQuantity()
        {
            ...
            $budget_category_id = BudgetCategory::where('id',
                $training->categoryItems()->first()->budget_category_id)->first()->id;
            switch ($budget_category_id) {
                case 1:
                    $this->calculateSuppliesTotalQuantity();
                    $cost_per_unit = $this->training_cost_per_unit == null ? 0 : $this->training_cost_per_unit;
                    ...
                    foreach ($this->trainings as $key => $training) {
                        if ($training['particular_id'] == $this->training_particular_id && $training['uom'] == $this->training_uom && $training['remarks'] == $this->training_remarks) {
                            ...
                            $draft_items = $this->record->fundAllocations->where('wpf_type_id',
                                $this->wfp_param)->where('supplemental_quarter_id',
                                $this->supplementalQuarterId)->first()->fundDrafts->first()->draft_items->where('particular_id',
                                $this->training_particular_id)->where('uom', $this->training_uom)->where('remarks',
                                $this->training_remarks)->first();
        ...
        public function updatedMachineQuantity()
        {
            ...
            $budget_category_id = BudgetCategory::where('id',
                $machine->categoryItems()->first()->budget_category_id)->first()->id;
                    ...
                    foreach ($this->machines as $key => $machine) {
                        if ($machine['particular_id'] == $this->machine_particular_id && $machine['uom'] == $this->machine_uom && $machine['remarks'] == $this->machine_remarks) {
                            ...
                            $draft_items = $this->record->fundAllocations->where('wpf_type_id',
                                $this->wfp_param)->where('supplemental_quarter_id',
                                $this->supplementalQuarterId)->first()->fundDrafts->first()->draft_items->where('particular_id',
                                $this->machine_particular_id)->where('uom', $this->machine_uom)->where('remarks',
                                $this->machine_remarks)->first();
        ...
        public function updatedBuildingQuantity()
        {
            ...
            $budget_category_id = BudgetCategory::where('id',
                $building->categoryItems()->first()->budget_category_id)->first()->id;
                    ...
                    foreach ($this->buildings as $key => $building) {
                        if ($building['particular_id'] == $this->building_particular_id && $building['uom'] == $this->building_uom && $building['remarks'] == $this->building_remarks) {
                            ...
                            $draft_items = $this->record->fundAllocations->where('wpf_type_id',
                                $this->wfp_param)->where('supplemental_quarter_id',
                                $this->supplementalQuarterId)->first()->fundDrafts->first()->draft_items->where('particular_id',
                                $this->building_particular_id)->where('uom', $this->building_uom)->where('remarks',
                                $this->building_remarks)->first();
        ...
        public function updatedPsQuantity()
        {
            ...
            $budget_category_id = BudgetCategory::where('id',
                $ps->categoryItems()->first()->budget_category_id)->first()->id;
            switch ($budget_category_id) {
                case 1:
                    $this->calculateSuppliesTotalQuantity();
                    $cost_per_unit = $this->ps_cost_per_unit == null ? 0 : $this->ps_cost_per_unit;
                    ...
                    foreach ($this->ps as $key => $ps) {
                        if ($ps['particular_id'] == $this->ps_particular_id && $ps['uom'] == $this->ps_uom && $ps['remarks'] == $this->ps_remarks) {
                            ...
                            $draft_items = $this->record->fundAllocations->where('wpf_type_id',
                                $this->wfp_param)->where('supplemental_quarter_id',
                                $this->supplementalQuarterId)->first()->fundDrafts->first()->draft_items->where('particular_id',
                                $this->ps_particular_id)->where('uom', $this->ps_uom)->where('remarks',
                                $this->ps_remarks)->first();
        ...
        public function submit()
        {
            if ($this->is_supplemental) {
                ...
                DB::beginTransaction();
                if (!$is_not_valid) {
                    $sumAllocated = $this->record->fundAllocations->where('wpf_type_id',
                        $this->wfp_param)->where('supplemental_quarter_id',
                        $this->supplementalQuarterId)->sum('initial_amount');
                    // $sumTotal = $this->record->fundAllocations
                    if ($this->record->fundAllocations->where('wpf_type_id',
                        $this->wfp_param)->where('supplemental_quarter_id',
                        $this->supplementalQuarterId)->first()->fundDrafts()->first()->draft_items()->exists()) {
                        $sumTotal = $this->record->fundAllocations->where('wpf_type_id',
                            $this->wfp_param)->where('supplemental_quarter_id',
                            $this->supplementalQuarterId)->first()->fundDrafts->first()->draft_items->sum('estimated_budget');
                    } else {
                        $sumTotal = array_sum(array_column($this->current_balance, 'current_total'));
                    }
                    $sumBalance = $sumAllocated - $sumTotal;
                    //if wfp already exist
                    if ($this->record->wfp()->where('wpf_type_id',
                        $this->wfp_type->id)->where('supplemental_quarter_id',
                        $this->supplementalQuarterId)->where('fund_cluster_id', $this->wfp_fund->id)->exists()) {
                        $wfp = $this->record->wfp()->where('wpf_type_id',
                            $this->wfp_type->id)->where('supplemental_quarter_id',
                            $this->supplementalQuarterId)->where('fund_cluster_id', $this->wfp_fund->id)->first();
                        $wfp->update([
                ...
                if ($this->wfp_fund->id === 1 || $this->wfp_fund->id === 3) {
                    ...
                    if (array_sum(array_column($this->current_balance,
                            'current_total')) > $this->record->fundAllocations->where('wpf_type_id',
                            $this->wfp_param)->sum('initial_amount')) {
                        $is_not_valid = true;
                    }
                }
                ...
                if (!$is_not_valid) {
                    ...
                    // $sumTotal = $this->record->fundAllocations
                    if ($this->record->fundAllocations->where('wpf_type_id',
                        $this->wfp_param)->first()->fundDrafts()->first()->draft_items()->exists()) {
                        $sumTotal = $this->record->fundAllocations->where('wpf_type_id',
                            $this->wfp_param)->first()->fundDrafts->first()->draft_items->sum('estimated_budget');
                    } else {
                        $sumTotal = array_sum(array_column($this->current_balance, 'current_total'));
                    }
                    $sumBalance = $sumAllocated - $sumTotal;
                    ...
                    //if wfp already exist
                    if ($this->record->wfp()->where('wpf_type_id', $this->wfp_type->id)->where('fund_cluster_id',
                        $this->wfp_fund->id)->exists()) {
                        $wfp = $this->record->wfp()->where('wpf_type_id', $this->wfp_type->id)->where('fund_cluster_id',
                            $this->wfp_fund->id)->first();
                        $wfp->update([
                            'cost_center_id' => $this->record->id,
                            'wpf_type_id' => $this->wfp_type->id,
                            'fund_cluster_id' => $this->wfp_fund->id,
        ...
        public function deleteSupply($index)
        {
            if ($this->is_supplemental) {
                if (isset($this->supplies[$index])) {
                    ...
                    $fund_draft = $this->fund_allocations->where('supplemental_quarter_id',
                        $this->supplementalQuarterId)->first()->fundDrafts->first();
                    $draft = $this->record->fundAllocations->where('wpf_type_id',
                        $this->wfp_param)->where('supplemental_quarter_id',
                        $this->supplementalQuarterId)->first()->fundDrafts()->first();
                    $draft_amount = FundDraftAmount::where('fund_draft_id', $draft->id)->where('category_group_id',
                        $title_group)->first();
                    $draft_item = $fund_draft->draft_items->where('title_group', $title_group)->where('particular_id',
                        $particular_id)->where('uom', $uom)
                ...
                if (isset($this->supplies[$index])) {
                    ...
                    // $draft_item = FundDraftItem::whereHas('fundDraft')->where('particular_id', $particular_id)->where('uom', $uom)
                    // ->where('remarks', $remarks)->where('supply_code', $supply_code)->first();
                    $draft_amount = $fund_draft->draft_amounts->where('category_group_id',
                        $title_group)->where('fund_draft_id', $fund_draft->id)->first();
                    foreach ($this->current_balance as $key => $item) {
        ...
        public function deleteTraining($index)
        {
            if ($this->is_supplemental) {
                ...
                if (isset($this->trainings[$index])) {
                    ...
                    $draft = $this->record->fundAllocations->where('wpf_type_id',
                        $this->wfp_param)->where('is_supplemental', 0)->first()->fundDrafts()->first();
                    $draft_amount = FundDraftAmount::where('fund_draft_id', $draft->id)->where('category_group_id',
                        $title_group)->first();
                    $draft_item = $fund_draft->draft_items->where('title_group', $title_group)->where('particular_id',
                        $particular_id)->where('uom', $uom)
