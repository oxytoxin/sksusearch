        ...
        public function mount($record, $wfpType, $isForwarded)
        {
            ...
            if ($isForwarded) {
                        ...
                        $query->where('wpf_type_id', $wfpType)->where(function ($query) use ($wfpType) {
                            if ($this->supplementalQuarterId == 1) {
                                ...
                                $query->where('is_supplemental', 0)->orWhere('supplemental_quarter_id', '<=',
                                    (int) $this->supplementalQuarterId);
                            }
                        });
                ...
                $this->category_groups_supplemental = CategoryGroup::whereHas('fundAllocations', function ($query) {
                    $query->where('cost_center_id', $this->record->id)->where('is_supplemental',
                ...
                if (count($this->prev_allocations) > 0) {
                    ...
                    $this->fundAllocation = $this->record->fundAllocations->where('wpf_type_id',
                        $wfpType)->where('supplemental_quarter_id', $this->supplementalQuarterId)->first();
                    ...
                    $this->supplemental_allocation_description = $this->record->fundAllocations->where('wpf_type_id',
                        $wfpType)->where('supplemental_quarter_id', $this->supplementalQuarterId)->first()->description;
                    ...
                    $this->fundAllocation = $this->record->fundAllocations->where('supplemental_quarter_id',
                        $this->supplementalQuarterId)->first();

                    $this->balance_164 = $this->fundInitialAmount;

                    $this->supplemental_allocation_description = $this->record->fundAllocations->where('supplemental_quarter_id',
                        $this->supplementalQuarterId)->first()->description;
                    $this->supplemental_allocation = $this->record->fundAllocations->where('supplemental_quarter_id',
                        $this->supplementalQuarterId)->first()->initial_amount;
                    $this->sub_total_164 = $this->balance_164 + $this->supplemental_allocation;
                    $this->balance_164_q1 = $this->sub_total_164 - array_sum($this->programmed_supplemental);
                }
                ...
                foreach ($this->record->fundAllocations->where('wpf_type_id', $wfpType)->where('is_supplemental',
                    0) as $allocation) {
                    $this->allocations[$allocation->category_group_id] = $allocation->initial_amount;
                }
                ...
                //supplemental
                foreach ($this->record->wfp->where('wpf_type_id', $this->selectedType)->where('is_supplemental',
                    1)->where('cost_center_id', $this->record->id) as $wfp) {
                    foreach ($wfp->wfpDetails as $allocation) {
                        if (!isset($this->programmed_supplemental[$allocation->category_group_id])) {
                            $this->programmed_supplemental[$allocation->category_group_id] = 0;
                        }
