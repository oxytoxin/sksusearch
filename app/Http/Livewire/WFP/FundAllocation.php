...
use Filament\Tables\Concerns\InteractsWithTable;
    ...
    class FundAllocation extends Component implements HasForms, HasTable
    {
        ...
        public $filter_is_supplemental = false;

        public $supplementalQuarterId = null;
        protected $queryString = ['filter_is_supplemental', 'supplementalQuarterId'];
                    ...
                    'fundAllocations' => function ($query) {
                        $query->where('wpf_type_id', $this->data['wfp_type'])->where(function ($query) {
                            if (!is_null($this->supplementalQuarterId)) {
                                $query->where('is_supplemental', 0)->orWhere('supplemental_quarter_id', '<=',
                ...
                Tables\Columns\TextColumn::make('fundAllocations.amount')
                    ...
                    ->formatStateUsing(function ($record) {
                        if (in_array($record->fundClusterWFP->id, [1, 3, 9])) {
                            $sum = $record->fundAllocations->where('cost_center_id', $record->id)
                                ->where('is_supplemental', $this->filter_is_supplemental)
                                ->where('supplemental_quarter_id', $this->supplementalQuarterId)
                                ->where('wpf_type_id', $this->data['wfp_type'])->sum('initial_amount');
                            return '₱ '.number_format($sum, 2);
                        } else {
                            return '₱ '.number_format($record->fundAllocations->where('cost_center_id', $record->id)
                                    ->where('is_supplemental', $this->filter_is_supplemental)
                                    ->where('wpf_type_id', $this->data['wfp_type'])->sum('initial_amount'), 2);
                        }
                    }),
        ...
        protected function getTableActions(): array
        {
            return [
                Action::make('allocate_fund')
                    ...
                    ->label('Allocate Fund')
                    ->button()
                    ->url(fn(CostCenter $record
                    ): string => $this->filter_is_supplemental ? route('wfp.add-supplemental-fund', [
                        'record' => $record, 'wfpType' => $this->data['wfp_type'], 'isForwarded' => 0,
                        'supplementalQuarterId' => $this->supplementalQuarterId
                    ]) : route('wfp.allocate-funds', $record))
                    ->requiresConfirmation()
                    ->visible(function ($record) {
                        if ($this->filter_is_supplemental) {
                            $current_allocation = $record->fundAllocations->where('supplemental_quarter_id',
                                $this->supplementalQuarterId)->first();
                ...
                Action::make('edit_fund')
                    ...
                    ->visible(function ($record) {
                        if (!$this->filter_is_supplemental) {
                            return $record->fundAllocations()->whereIsSupplemental(0)->whereIsLocked(false)->exists() && $record->fundAllocations()->where('wpf_type_id',
                ...
                ActionGroup::make([
                    Action::make('unlock_fund')
                        ...
                        ->visible(function ($record) {
                            if ($this->filter_is_supplemental) {
                                ...
                                return $record->fundAllocations()->whereIsSupplemental(0)->where('wpf_type_id',
                                        $this->data['wfp_type'])->whereIsLocked(true)->exists() && !$this->isPresident;
                            }
                        }),
                    ViewAction::make('view_allocation')
                        ...
                        ->icon('ri-eye-line')
                        ->url(fn(CostCenter $record
                        ): string => $this->filter_is_supplemental ? route('wfp.view-supplemental-fund', [
                            'record' => $record, 'wfpType' => $this->data['wfp_type'],
                            'isForwarded' => $record->wfp()->where('is_supplemental', 0)->exists() ? 0 : 1,
                            'supplementalQuarterId' => $this->supplementalQuarterId
                        ]) : route('wfp.view-allocated-funds',
                            ['record' => $record, 'wfpType' => $this->data['wfp_type']]))
                        ->visible(function ($record) {
                            if ($this->filter_is_supplemental) {
                                $current_allocation = $record->fundAllocations->where('supplemental_quarter_id',
                    ...
                    Action::make('edit_supplemental')
                        ...
                        ->button()
                        ->color('warning')
                        ->url(fn(CostCenter $record): string => route('wfp.edit-supplemental-funds-q1', [
                            'record' => $record, 'wfpType' => $this->data['wfp_type'],
                            'supplementalQuarterId' => $this->supplementalQuarterId,
                            'isForwarded' => $record->wfp()->where('is_supplemental', 0)->exists() ? 0 : 1
                        ]))
                        ->visible(function (CostCenter $record) {
                            if ($this->filter_is_supplemental) {
                                if ($record->wfp->where('supplemental_quarter_id',
                                    $this->supplementalQuarterId)->first()) {
                                    return $record->wfp->where('supplemental_quarter_id',
                                            $this->supplementalQuarterId)->first()->is_approved !== 1;
                                }
                                return true;
                            } else {
                                return count($record->wfp->where('is_supplemental',
                                        0)) === 0 || $record->wfp->where('is_supplemental',
                                        0)->first()->is_approved !== 1;
                            }
                        }),
                ]),
                Action::make('forward_balance')
                    ...
                    ->modalSubheading(function (CostCenter $record) {
                        if ($record->fundAllocations()->exists()) {
                            $toBeForwarded = $record->fundAllocations->where('wpf_type_id',
                                $this->data['wfp_type'])->where('is_supplemental', 0)->first()->initial_amount;
                        } else {
                            $toBeForwarded = 0;
                        }


                        return 'The balance of ₱ '.number_format($toBeForwarded,
                                2).' will be forwarded. Are you sure you want to proceed?';
                    })
                    ->action(function (CostCenter $record) {
                        if ($record->fundAllocations()->exists()) {
                            if ($record->fundAllocations->first()->fundDrafts()->exists()) {
                                \DB::statement('SET FOREIGN_KEY_CHECKS=0;');
                                if ($record->fundAllocations->first()->fundDrafts && $record->fundAllocations->first()->fundDrafts->first()->draft_items != null) {
                                    $record->fundAllocations->first()->fundDrafts->first()->draft_items()->delete();
                                }
                                if ($record->fundAllocations->first()->fundDrafts && $record->fundAllocations->first()->fundDrafts->first()->draft_amounts != null) {
                                    $record->fundAllocations->first()->fundDrafts->first()->draft_amounts()->delete();
                                }
                                $record->fundAllocations->first()->fundDrafts()->delete();
                                \DB::statement('SET FOREIGN_KEY_CHECKS=1;');
                            }
                        }
                        return redirect()->route('wfp.add-supplemental-fund',
                            ['record' => $record, 'wfpType' => $this->data['wfp_type'], 'isForwarded' => 1]);
                    })
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        if ($data['wfp_status'] != 20) {
                            if ($data['wfp_status'] == null) {
                                return $query->whereDoesntHave('wfp', function ($query) {
                                ...
                                return $query->whereHas('wfp', function ($query) use ($data) {
                                    $query->where('wpf_type_id', $this->data['wfp_type'])->where('is_approved',
                                        $data['wfp_status'])->where('is_supplemental', false);
                                });
                            }
                        }
                        return $query;
