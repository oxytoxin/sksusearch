<?php
    ...
    class GeneratePpmpQ1 extends Component
    {
        ...
        public function sksuPpmp()
        {
            ...
            category_groups.name as name, SUM(fund_allocations.initial_amount) as total_allocated'
                ...
                ->groupBy('fund_allocations.wpf_type_id', 'category_groups.id', 'category_groups.name',
                    'fund_allocations.is_supplemental')
                ->get();


            $this->non_supplemental_fund_allocation = $tem_fund_allocation->where('is_supplemental', 0);
            $supplemental_fund_allocation = $tem_fund_allocation->where('is_supplemental',
                1)->pluck('category_group_id')->toArray();

            $this->fund_allocation = $tem_fund_allocation->filter(function ($allocation) use (
                $supplemental_fund_allocation
            ) {
                return $allocation->is_supplemental || (!in_array($allocation->category_group_id,
                            $supplemental_fund_allocation) && $allocation->is_supplemental == 0);
            });

            $this->ppmp_details = WfpDetail::whereHas('wfp', function ($query) {
                $query->where('fund_cluster_id', 1)->where('wpf_type_id', $this->selectedType)
            ...
            switch ($this->title) {
                case 'Sultan Kudarat State University':
                    ...
                    return \Excel::download(new PreExport($this->selectedType, $this->fund_allocation,
                        $this->ppmp_details, $this->total_allocated, $this->total_programmed, $this->balance,
                        $this->non_supplemental_fund_allocation, $this->forwarded_ppmp_details,
                        $this->non_supplemental_total_programmed, $this->is_q1, $this->activeButton),
                        '101-'.$this->title.'.xlsx');
                    break;
        ...
        public function gasPpmp()
        {
            ...
            $supplemental_fund_allocation = $temp_fund_allocation->where('is_supplemental',
                1)->pluck('category_group_id')->toArray();

            $this->fund_allocation = $temp_fund_allocation->filter(function ($allocation) use (
                $supplemental_fund_allocation
            ) {
                return $allocation->is_supplemental || (!in_array($allocation->category_group_id,
                            $supplemental_fund_allocation) && $allocation->is_supplemental == 0);
        ...
        public function generateSksuppmp($fcwpsId, $title)
        {
            ...
            $temp_fund_allocation = FundAllocation::selectRaw('wpf_type_id,is_supplemental, mfo_fees.id as mfo_fee_id, mfo_fees.name as name, SUM(initial_amount) as total_allocated')
                ...
                ->where('mfo_fees.fund_cluster_id', $fcwpsId)
            ...
            $this->non_supplemental_fund_allocation = $temp_fund_allocation->where('is_supplemental', 0);
            $supplemental_fund_allocation = $temp_fund_allocation->where('is_supplemental',
                1)->pluck('mfo_fee_id')->toArray();

            $this->fund_allocation = $temp_fund_allocation->filter(function ($allocation) use (
                $supplemental_fund_allocation
            ) {
                return $allocation->is_supplemental || (!in_array($allocation->mfo_fee_id,
                            $supplemental_fund_allocation) && $allocation->is_supplemental == 0);
            });
            ...
            $this->total_programmed = WfpDetail::whereHas('wfp', function ($query) use ($fcwpsId) {
                $query->where('wpf_type_id', $this->selectedType)->where('fund_cluster_id', $fcwpsId)
                    ->where('is_supplemental', 1)->where('is_approved', 1);
            })->select(DB::raw('SUM(cost_per_unit * total_quantity) as total_budget'))->first();
            $this->balance = $this->total_allocated - $this->total_programmed->total_budget;
        ...
        public function generateSksuppmpPerCostCenterMfo($fcwpsId, $ccMfoId, $title)
        {
            ...
            $this->non_supplemental_total_programmed = WfpDetail::whereHas('wfp',
                function ($query) use ($fcwpsId, $ccMfoId) {
                    $query->where('wpf_type_id', $this->selectedType)->where('fund_cluster_id', $fcwpsId)
                        ->where('is_supplemental', 0)->where('is_approved', 1)
