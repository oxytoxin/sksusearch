    ...
    public $total;
     ...
     public function sksuPpmp()
     {
         ...
         $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
            $query->where('fund_cluster_w_f_p_s_id', 1)->whereIn('is_approved', [1, 500]);
         })
     ...
     public function gasPpmp()
     {
        ...
        $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
            $query->whereIn('is_approved', [1, 500])->where('fund_cluster_w_f_p_s_id', 1)->whereHas('costCenter', function($query) {
     ...
     public function sksuPpmp161()
     {
         ...
         $this->title = 'Sultan Kudarat State University';
        $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
            $query->where('fund_cluster_w_f_p_s_id', 3)->whereIn('is_approved', [1, 500]);
        })
        ->with(['supply', 'categoryItem'])  // Load both relationships
        ->selectRaw('supply_id, category_item_id, uacs_code, budget_category_id, uom, SUM(total_quantity) as total_quantity, cost_per_unit, SUM(cost_per_unit * total_quantity) as estimated_budget, JSON_ARRAYAGG(quantity_year) as merged_quantities')
        ->groupBy('supply_id', 'category_item_id', 'uacs_code', 'cost_per_unit', 'uom', 'budget_category_id')
     ...
     public function gasPpmp161()
     {
         ...
         $this->title = 'General Admission and Support Services';
        $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
            $query->whereIn('is_approved', [1, 500])->where('fund_cluster_w_f_p_s_id', 3)->whereHas('costCenter', function($query) {
                $query->where('m_f_o_s_id', 1);
                });
        })
        ->with(['supply', 'categoryItem'])  // Load both relationships
     ...
     public function sksuPpmp163()
     {
         ...
         $this->title = 'Sultan Kudarat State University';
        $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
            $query->where('fund_cluster_w_f_p_s_id', 2)->whereIn('is_approved', [1, 500]);
        })
        ->with(['supply', 'categoryItem'])  // Load both relationships
        ->selectRaw('supply_id, category_item_id, uacs_code, budget_category_id, uom, SUM(total_quantity) as total_quantity, cost_per_unit, SUM(cost_per_unit * total_quantity) as estimated_budget, JSON_ARRAYAGG(quantity_year) as merged_quantities')
        ->groupBy('supply_id', 'category_item_id', 'uacs_code', 'cost_per_unit', 'uom', 'budget_category_id')
     ...
     public function accessPpmp163()
     {
       ...
       $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
        $query->whereIn('is_approved', [1, 500])->where('fund_cluster_w_f_p_s_id', 2)->whereHas('costCenter', function($query) {
     ...
     public function sksuPpmp164T()
     {
         ...
         $this->title = 'Sultan Kudarat State University';
        $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
            $query->where('fund_cluster_w_f_p_s_id', 4)->whereIn('is_approved', [1, 500]);
        })
        ->with(['supply', 'categoryItem'])  // Load both relationships
        ->selectRaw('supply_id, category_item_id, uacs_code, budget_category_id, uom, SUM(total_quantity) as total_quantity, cost_per_unit, SUM(cost_per_unit * total_quantity) as estimated_budget, JSON_ARRAYAGG(quantity_year) as merged_quantities')
        ->groupBy('supply_id', 'category_item_id', 'uacs_code', 'cost_per_unit', 'uom', 'budget_category_id')
     ...
     public function gasPpmp164T()
     {
         ...
         $this->title = 'General Admission and Support Services';
        $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
            $query->whereIn('is_approved', [1, 500])->where('fund_cluster_w_f_p_s_id', 4)->whereHas('costCenter', function($query) {
                $query->where('m_f_o_s_id', 1);
                });
        })
        ->with(['supply', 'categoryItem'])  // Load both relationships
     ...
     public function lfPpmp164T()
     {
        ...
        foreach ($this->record as $record) {
          ...
          public function sksuPpmp164TN()
          {
              ...
              $this->title = 'Sultan Kudarat State University';
             $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
                 $query->where('fund_cluster_w_f_p_s_id', 7)->whereIn('is_approved', [1, 500]);
             })
             ->with(['supply', 'categoryItem'])  // Load both relationships
             ->selectRaw('supply_id, category_item_id, uacs_code, budget_category_id, uom, SUM(total_quantity) as total_quantity, cost_per_unit, SUM(cost_per_unit * total_quantity) as estimated_budget, JSON_ARRAYAGG(quantity_year) as merged_quantities')
             ->groupBy('supply_id', 'category_item_id', 'uacs_code', 'cost_per_unit', 'uom', 'budget_category_id')
          ...
          public function gasPpmp164TN()
          {
              ...
              $this->title = 'General Admission and Support Services';
             $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
                 $query->whereIn('is_approved', [1, 500])->where('fund_cluster_w_f_p_s_id', 7)->whereHas('costCenter', function($query) {
                     $query->where('m_f_o_s_id', 1);
                     });
             })
             ->with(['supply', 'categoryItem'])  // Load both relationships
     ...
     public function sksuPpmp164OSF()
     {
         ...
         $this->title = 'Sultan Kudarat State University';
        $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
            $query->where('fund_cluster_w_f_p_s_id', 5)->whereIn('is_approved', [1, 500]);
        })
        ->with(['supply', 'categoryItem'])  // Load both relationships
        ->selectRaw('supply_id, category_item_id, uacs_code, budget_category_id, uom, SUM(total_quantity) as total_quantity, cost_per_unit, SUM(cost_per_unit * total_quantity) as estimated_budget, JSON_ARRAYAGG(quantity_year) as merged_quantities')
        ->groupBy('supply_id', 'category_item_id', 'uacs_code', 'cost_per_unit', 'uom', 'budget_category_id')
     ...
     public function gasPpmp164OSF()
     {
         ...
         $this->title = 'General Admission and Support Services';
        $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
            $query->whereIn('is_approved', [1, 500])->where('fund_cluster_w_f_p_s_id', 5)->whereHas('costCenter', function($query) {
                $query->where('m_f_o_s_id', 1);
                });
        })
        ->with(['supply', 'categoryItem'])  // Load both relationships
     ...
     public function sksuPpmp164MF()
     {
         ...
         $this->title = 'Sultan Kudarat State University';
        $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
            $query->where('fund_cluster_w_f_p_s_id', 6)->whereIn('is_approved', [1, 500]);
        })
        ->with(['supply', 'categoryItem'])  // Load both relationships
        ->selectRaw('supply_id, category_item_id, uacs_code, budget_category_id, uom, SUM(total_quantity) as total_quantity, cost_per_unit, SUM(cost_per_unit * total_quantity) as estimated_budget, JSON_ARRAYAGG(quantity_year) as merged_quantities')
        ->groupBy('supply_id', 'category_item_id', 'uacs_code', 'cost_per_unit', 'uom', 'budget_category_id')
     ...
     public function gasPpmp164MF()
     {
         ...
         $this->title = 'General Admission and Support Services';
        $this->record = WfpDetail::where('is_ppmp', 1)->whereHas('wfp', function ($query) {
            $query->whereIn('is_approved', [1, 500])->where('fund_cluster_w_f_p_s_id', 6)->whereHas('costCenter', function($query) {
                $query->where('m_f_o_s_id', 1);
                });
        })
        ->with(['supply', 'categoryItem'])  // Load both relationships
