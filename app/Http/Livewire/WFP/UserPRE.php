        ...
        public function mount($record, $isSupplemental)
        {
            ...
            if (in_array($this->record->fundClusterWfp->id, [1, 3, 9])) {
                if ($isSupplemental) {
                    ...
                    $supplemental_fund_allocation = $temp_fund_allocation->where('supplemental_quarter_id',
                    ...
                    $this->fund_allocation = $temp_fund_allocation->filter(function ($item) use (
                        ...
                        if ($item->is_supplemental === 1) {
                            ...
                            if ($item->is_supplemental === 0 && !is_null($non_supplemental_fund_allocation->where('category_group_id',
                                    $item->category_group_id)->first()) && $supplemental_fund_allocation->where('category_group_id',
                                    $item->category_group_id)->first() === null) {
                                return $item;
                            } else {
                                return null;
                            }
                ...
                $this->fund_allocation = FundAllocation::where('is_supplemental',
                ...
                if ($isSupplemental) {
                    $temp_total_allocated = FundAllocation::where('is_supplemental', 0)->where('cost_center_id',
                        $this->cost_center->id)
                ...
                $this->total_allocated = FundAllocation::where('is_supplemental',
                    $isSupplemental)->where('cost_center_id', $this->cost_center->id)
                    ->where('initial_amount', '>', 0)->sum('initial_amount');
                $this->total_programmed = WfpDetail::whereHas('wfp', function ($query) use ($isSupplemental) {
                    $query->when($this->wfpType, function ($query) {
                        $query->where('wpf_type_id', $this->wfpType);
