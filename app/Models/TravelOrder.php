...
class TravelOrder extends Model
{
    ...
    private const SIGNATORY_PIVOT_COLUMNS = [
        'id',
        'is_approved',
        'role',
        'heading',
    ...
    {
        return $this->belongsToMany(User::class, 'travel_order_signatories', 'travel_order_id', 'user_id')
            ->using(TravelOrderSignatory::class)
            ->withPivot(self::SIGNATORY_PIVOT_COLUMNS)
            ->withTimestamps();
    }

    public function immediate_supervisors()
    {
        return $this->signatories()->wherePivot('role', 'immediate_supervisor');
    }

    public function recommending_approval()
    {
        return $this->signatories()->wherePivot('role', 'recommending_approval');
    }

    public function university_president()
    {
        return $this->signatories()->wherePivot('role', 'university_president');
    }
