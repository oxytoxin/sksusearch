...
class DisbursementVoucherWorkflowService
{
    ...
    public function forward(DisbursementVoucher $voucher, User $actor, ?string $remarks = null, array $options = []): DisbursementVoucher
    {
        ...
        return DB::transaction(function () use ($voucher, $actor, $remarks, $options) {
            // Capture the origin step before any movement so we know which slot just signed off.
            $originStepId = $voucher->current_step_id;
            ...
            $this->recordForwardApproval($voucher, $originStepId, $actor, $options);
    ...
    public function certify(DisbursementVoucher $voucher, array $options = []): DisbursementVoucher
    {
        ...
        return DB::transaction(function () use ($voucher, $options) {
            ...
            $this->recordAccountantApproval($voucher, $options);
    ...
    public function validateUacsAllocations(DisbursementVoucher $voucher, $allocations): void
    {
    ...
    /**
     * The acting OIC's own id (i.e. the `approved_by_oic_id` value) when the action was
     * performed as OIC, else null for a direct sign-off. This is the person who actually
     * signed; `slot_owner_id` (handled separately) records who they acted on behalf of.
     */
    private function resolveOicId(array $options, ?User $actor = null): ?int
    {
        if (! ($options['is_oic'] ?? false)) {
            return null;
        }

        return $actor?->id ?? ($options['actor']?->id ?? null) ?? auth()->id();
    }

    /**
     * Record per-role approval rows when forwarding crosses a sign-off step:
     *  - origin 4000  -> signatory slot (name frozen to the DV's signatory)
     *  - origin 15000 -> president slot (direct sign-off records the actor's own id;
     *    OIC sign-off records the slot owner whose block this belongs to)
     */
    private function recordForwardApproval(DisbursementVoucher $voucher, int $originStepId, User $actor, array $options): void
    {
        $approvedAt = now();
        $oicId = $this->resolveOicId($options, $actor);

        if ($originStepId === 4000) {
            $voucher->signatoryApproval()->updateOrCreate(
                ['role' => 'signatory'],
                [
                    'user_id' => $voucher->signatory_id,
        ...
        if ($originStepId === 15000) {
            $slotOwnerId = $oicId ? ($options['slot_owner_id'] ?? $actor->id) : $actor->id;

            $voucher->presidentApproval()->updateOrCreate(
                ['role' => 'president'],
                [
                    'user_id' => $slotOwnerId,
                    'approved_at' => $approvedAt,
    ...
    /**
     * Record the accountant (certifying) approval row. `slot_owner_id` freezes the
     * accountant position holder at sign-off when acting as OIC; otherwise the acting
     * user (the accountant who certified) is recorded.
     */
    private function recordAccountantApproval(DisbursementVoucher $voucher, array $options): void
    {
        $slotOwnerId = ($options['is_oic'] ?? false)
            ? ($options['slot_owner_id'] ?? ($options['actor']?->id ?? null) ?? auth()->id())
            : ($options['actor']?->id ?? auth()->id());

        $voucher->accountantApproval()->updateOrCreate(
            ['role' => 'accountant'],
            [
                'user_id' => $slotOwnerId,
                'approved_at' => now(),
                'approved_by_oic_id' => $this->resolveOicId($options),
            ],
        );
    }
