<div x-data>
    <div class="p-4">
        <div class="flex justify-center">
            <button @click="showPrintable = true" wire:click="sksuPpmp101Con"
                class="bg-green-800 hover:bg-green-700 text-white font-bold py-1.5 px-8 rounded-lg">
                SKSU 101 - Continuing
            </button>
        </div>
        <div class="flex justify-center space-x-4 mt-3">
            <button @click="showPrintable = true" wire:click="gasPpmp101Con"
            ...
            <button @click="showPrintable = true" wire:click="hesPpmp101Con"
                class="bg-green-800 hover:bg-green-700 text-white font-bold py-1.5 px-3 rounded-lg">
                Higher Education Services (HES)
            </button>
            <button @click="showPrintable = true" wire:click="aesPpmp101Con"
                class="bg-green-800 hover:bg-green-700 text-white font-bold py-1.5 px-3 rounded-lg">
                Advanced Education Services (AES)
            </button>
            <button @click="showPrintable = true" wire:click="rdPpmp101Con"
                class="bg-green-800 hover:bg-green-700 text-white font-bold py-1.5 px-3 rounded-lg">
                Research and Development (RD)
            </button>
            <button @click="showPrintable = true" wire:click="extensionPpmp101Con"
                class="bg-green-800 hover:bg-green-700 text-white font-bold py-1.5 px-3 rounded-lg">
                Extension Services (ES)
            </button>
            <button @click="showPrintable = true" wire:click="lfPpmp101Con"
                class="bg-green-800 hover:bg-green-700 text-white font-bold py-1.5 px-3 rounded-lg">
    ...
    <div x-show="showPrintable" class="bg-gray-50">
        @if ($is_active)
            <div class="flex justify-end p-4 space-x-3">
                ...
                <button wire:click="export101" type="button"
                ...
                <div>
                    ...
                    @else
                        <table class="w-full mt-4">
                            <thead>
                                <tr>
                                    @if ($is_q1 && in_array($activeButton, ['sksuPpmp101Con']))
                                ...
                                @if ($is_q1 && in_array($activeButton, ['sksuPpmp101Con']))
                                    ...
                                    <td class="border border-black text-left font-semibold p-1">
                                        <div class="flex justify-between">
                                            <span>₱</span>
                                            <span>
                                                {{ $non_supplemental_fund_allocation->sum('total_allocated') > 0 ? number_format($non_supplemental_fund_allocation->sum('total_allocated') - $forwarded_ppmp_details->sum('total_budget') + ($total_allocated ?? 0), 2) : number_format($forwarded_ppmp_details->sum('total_budget') + ($total_allocated ?? 0), 2) }}</span>
                                        </div>
                                    </td>
                                @endif
