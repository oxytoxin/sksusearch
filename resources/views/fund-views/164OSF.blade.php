    ...
    <div x-show="showPrintable" class="bg-gray-50">
        @if ($is_active)
            <div class="flex justify-end p-4 space-x-4">
                ...
                <button wire:click="export164OSF" type="button"
                    class="flex hover:bg-green-600 p-2 bg-green-700 rounded-md font-light capitalize text-white text-sm">
                    Export Excel
                </button>
                <a type="button" target="_blank"
                    href="/export/cost-center?supplementalQuarterId={{ $supplementalQuarterId }}&is_supplemental={{ $is_q1 ? 1 : 0 }}&fund_cluster_id=5&wfp_type_id={{ $selectedType }}&m_f_o_s_id={{ $mfosId }}&fileName={{ str_replace(' ', '_', $title) }}.xlsx"
                    class="flex hover:bg-green-600 p-2 bg-green-700 rounded-md font-light capitalize text-white text-sm">
                    Cost Center Export
                </a>
            </div>
