<div class="space-y-4">
    <div class="flex items-center justify-between">
        <h2 class="text-lg font-semibold text-primary-600">Create Batch Transmittal</h2>
    ...
    <div class="rounded-lg bg-white p-4 shadow-sm">
        <div class="mb-3 flex items-center justify-between">
            <h3 class="font-semibold text-gray-700">Available Vouchers ({{ $this->forwardableDvs->count() }})</h3>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                ...
                <tbody>
                    @forelse ($this->forwardableDvs as $dv)
                        <tr class="border-b hover:bg-gray-50 {{ in_array($dv->id, $selectedDvs) ? 'bg-green-50' : '' }}">
                            <td class="px-3 py-2">
                                <input type="checkbox"
                                       wire:click="toggleDv({{ $dv->id }})"
                                       {{ in_array($dv->id, $selectedDvs) ? 'checked' : '' }}
                                       class="rounded border-gray-300 text-primary-600 focus:ring-primary-500">
                            </td>
                            <td class="px-3 py-2 font-mono text-xs">{{ $dv->tracking_number }}</td>
                            <td class="px-3 py-2">{{ $dv->payee }}</td>
                            <td class="px-3 py-2 max-w-xs truncate">{{ $dv->disbursement_voucher_particulars->pluck('purpose')->join('; ') }}</td>
                            <td class="px-3 py-2 whitespace-nowrap">₱{{ number_format($dv->disbursement_voucher_particulars->sum('amount'), 2) }}</td>
                            <td class="px-3 py-2">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-3 py-8 text-center text-gray-400">No forwardable vouchers in this office.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    {{-- Submit --}}
    @if (count($selectedDvs) > 0)
        <div class="flex items-center justify-between rounded-lg bg-white p-4 shadow-sm">
            <span class="text-sm font-semibold text-gray-700">{{ count($selectedDvs) }} DV(s) selected</span>
            <button wire:click="createAndForward"
                    wire:loading.attr="disabled"
                    onclick="return confirm('Create and forward batch with {{ count($selectedDvs) }} DV(s)?')"
