<div class="p-8" x-data>
    <div class="flex justify-end w-full px-2">
        <button class="flex px-4 py-2 text-center rounded-md bg-primary-700 text-primary-100 hover:bg-primary-900 hover:shadow-primary-600 hover:shadow-sm"
                @click="printOut($refs.dvPrint.outerHTML)">
    ...
    <div class="bg-white p-4 rounded-lg font-serif print:block print:rounded-none" x-ref="dvPrint">
        <div class="text-center w-full">
            <h1 class="text-sm font-extrabold tracking-wide"><strong>ITINERARY OF TRAVEL</strong></h1>
            ...
            <table class="w-full border-black border">
                ...
                <tr class="text-sm border border-black">
                    ...
                    <th class="font-normal border border-black" colspan="2" rowspan="2">Place to be visited
                        (Destination)
                    </th>
                ...
                @foreach ($itinerary->coverage as $coverage)
                    ...
                    @foreach (collect($itinerary_entries)->where('date', Carbon\Carbon::make($coverage['date'])) as $itinerary_entry)
                        <tr class="text-right border print:text-12 border-black">
                            ...
                            <td class="border-x min-w-[12rem] py-2 align-baseline border-black text-center"
                                colspan="2">{{ $itinerary_entry->place }}</td>
