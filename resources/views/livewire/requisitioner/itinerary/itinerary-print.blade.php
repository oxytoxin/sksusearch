<div class="p-8">
    <div class="flex justify-end w-full px-2">
        <button
            class="flex px-4 py-2 text-center rounded-md bg-primary-700 text-primary-100 hover:bg-primary-900 hover:shadow-sm hover:shadow-primary-600"
            onclick="printDiv('dvPrint')">
    ...
    <div class="print:block print:w-[220mm] print:h-[297mm] print:max-w-[220mm] print:max-h-[297mm] font-serif print:font-serif bg-white rounded-lg print:rounded-none"
        id="dvPrint">
        <div class="grid grid-cols-10 p-2 m-2 border-collapse print:text-12">
            <div class="col-span-10 text-center"><strong
                    class="text-sm font-extrabold tracking-wide uppercase print:text-sm">Itinerary of Travel</strong>
            </div>
            <div class="col-span-8">&nbsp</div>
            <div class="col-span-2 col-start-9 row-span-2 ">
                <div class="flex flex-col ">
                    <img class="w-auto mx-auto h-14"
                        src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data={{ $travel_order->tracking_code }}"
                        alt="N/A">
            ...
            <div class="col-span-5 px-2 py-1 border-t border-black border-x">Name: <span
                    class="font-extrabold">{{ auth()->user()->name }}</span></div>
            <div class="col-span-5 px-2 py-1 border-t border-r border-black">Date of Travel: <span
                    class="font-extrabold print:text-10">{{ $travel_order->date_from->format('M d, Y') }} to
                    {{ $travel_order->date_to->format('M d, Y') }}</span></div>
            <div class="col-span-5 px-2 py-1 border-black border-x">Position: <span
            ...
            <div
                class="grid justify-center grid-cols-2 col-span-2 grid-rows-2 row-span-2 text-sm text-center border-r border-black print:text-12 border-y">
            ...
            <div class="grid grid-cols-10 col-span-10 text-center border-b border-black border-x print:text-12">
                ...
                @foreach ($itinerary->itinerary_entries as $entry)
                    @if ($loop->index == 0)
                        ...
                        <div class="col-span-1 px-2 py-1 text-right border-r border-black">
                            {{ $entry->itinerary->coverage[0]['per_diem'] }}</div>
                        ...
                        <div class="col-span-1 px-2 py-1 text-right ">
                            {{ $entry->itinerary->coverage[0]['per_diem'] + $entry->other_expenses + $entry->transportation_expenses }}
                        </div>
                    @else
                        <div class="col-span-1 px-2 py-1 border-t border-r border-black">
                            {{ $entry->date->format('M d, Y') }}</div>
                        ...
                        <div class="col-span-1 px-2 py-1 text-right border-t border-r border-black">
                            {{ $entry->transportation_expenses }}</div>
                        <div class="col-span-1 px-2 py-1 text-right border-t border-r border-black">
                            {{ $entry->itinerary->coverage[$loop->index]['per_diem'] }}
                        </div>
                        <div class="col-span-1 px-2 py-1 text-right border-t border-r border-black">
                            {{ $entry->other_expenses }}</div>
                        <div class="col-span-1 px-2 py-1 text-right border-t border-black">
                            {{ $entry->itinerary->coverage[$loop->index]['per_diem'] + $entry->other_expenses + $entry->transportation_expenses }}
                        </div>
                    @endif
                @endforeach
            </div>
            ...
            <div class="col-span-3 col-start-7 text-center ">
                <div class="flex flex-col w-full pt-5 border-t-2 border-black">
                    <span
                        class="font-extrabold capitalize print:text-sm">{{ $itinerary->travel_order->signatories()->first()->name }}</span>
                    <span
                        class="text-xs font-extrabold capitalize print:text-10">{{ $itinerary->travel_order->signatories()->first()->employee_information->position->description }}
                        <span class="lowercase">of</span>
                        {{ $itinerary->travel_order->signatories()->first()->employee_information->office->name }}</span>
                </div>
            </div>
