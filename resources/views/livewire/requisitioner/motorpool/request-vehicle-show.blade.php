            ...
            <div
                ...
                <div class="flex flex-wrap items-center justify-between -mt-4 -ml-4 sm:flex-nowrap">
                    <div class="w-full mt-4 ml-4">
                        ...
                        <p class="mt-1 text-sm text-primary-500">Travel Order :
                            {{ $request->travel_order_id == null ? 'No' : 'Yes' }}</p>
                        @php
                            $earliestDate = App\Models\RequestScheduleTimeAndDate::where(
                                'request_schedule_id',
                                $request->id,
                            )->min('travel_date');
                            $latestDate = App\Models\RequestScheduleTimeAndDate::where(
                                'request_schedule_id',
                                $request->id,
                            )->max('travel_date');
                            $requestScheduleTime = App\Models\RequestScheduleTimeAndDate::where(
                                'request_schedule_id',
                                $request->id,
                            )->first();
                        @endphp
                        ...
                        @if (
                            $is_president &&
                                ...
                                ($request->travel_order?->is_fully_approved || !$request->travel_order))
                        ...
                        @if ($is_motorpool_head)
                            @if ($request->driver_id == null && $request->vehicle_id != null)
                                <a class="flex float-right mt-4 mx-2 px-4 py-2 text-sm rounded-full bg-primary-600 text-primary-100 hover:text-primary-100 hover:bg-primary-900 active:ring-primary-700 w-fit active:ring-2 active:ring-offset-2"
                                    ...
                                    <svg class="w-5 h-auto" class="w-6 h-6" xmlns="http://www.w3.org/2000/svg"
                            ...
                            @else
                                <a class="pointer-events-none flex float-right mt-4 mx-2 px-4 py-2 text-sm rounded-full bg-primary-400 text-primary-100 hover:text-primary-100 hover:bg-primary-900 active:ring-primary-700 w-fit active:ring-2 active:ring-offset-2"
                                    wire:click="$set('assignDriverModal',true)" target="_blank">
                                    <svg class="w-5 h-auto" class="w-6 h-6" xmlns="http://www.w3.org/2000/svg"
                                        fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            @else
                                <a class="pointer-events-none flex float-right mx-2 mt-4 px-4 py-2 text-sm rounded-full bg-primary-400 text-primary-100 hover:text-primary-100 hover:bg-primary-900 active:ring-primary-700 w-fit active:ring-2 active:ring-offset-2"
                                    wire:click="$set('assignVehicleModal',true)" target="_blank">
                                    <svg class="w-5 h-auto" class="w-6 h-6" xmlns="http://www.w3.org/2000/svg"
                                        fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            @if ($request->driver_id != null && $request->vehicle_id != null)
                                @if (!$request->fuel_request()->exists())
                                    <a class="flex float-right mx-2 mt-4 px-4 py-2 text-sm rounded-full bg-primary-600 text-primary-100 hover:text-primary-100 hover:bg-primary-900 active:ring-primary-700 w-fit active:ring-2 active:ring-offset-2"
                                        href="{{ route('motorpool.request.fuel-request', $request) }}" target="_blank">

                                        <svg class="w-5 h-auto" xmlns="http://www.w3.org/2000/svg" fill="none"
                                ...
                                @else
                                    <a class="pointer-events-none flex float-right mx-2 mt-4 px-4 py-2 text-sm rounded-full bg-primary-400 text-primary-100 hover:text-primary-100 hover:bg-primary-900 active:ring-primary-700 w-fit active:ring-2 active:ring-offset-2"
                                        href="{{ route('motorpool.request.fuel-request', $request) }}"
                                        target="_blank">
                                ...
                                @if ($request->fuel_request()->exists())
                                    <a class="flex float-right mx-2 mt-4 px-4 py-2 text-sm rounded-full bg-primary-600 text-primary-100 hover:text-primary-100 hover:bg-primary-900 active:ring-primary-700 w-fit active:ring-2 active:ring-offset-2"
                                        href="{{ route('motorpool.request.fuel-request-slip', $request->fuel_request->id) }}"
                                        target="_blank">
                                        <svg class="w-5 h-auto" xmlns="http://www.w3.org/2000/svg" fill="none"
                                            viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round"
    ...
    <x-modal.card title="Assign Vehicle" align="center" blur wire:model.defer="assignVehicleModal">
        ...
        <x-slot name="footer">
            <div class="flex justify-end gap-x-4">
                <div class="flex">
                    <x-button flat label="Cancel" x-on:click="close" />
                    <x-button primary label="Save" spinner="assignVehicle({{ $request->id }})"
                        wire:click="assignVehicle({{ $request->id }})" />
                </div>
            </div>
        </x-slot>
    </x-modal.card>
    ...
    <x-modal.card title="Change Vehicle" align="center" blur wire:model.defer="modifyVehicleModal">
        ...
        <x-slot name="footer">
            <div class="flex justify-end gap-x-4">
                <div class="flex">
                    <x-button flat label="Cancel" x-on:click="close" />
                    <x-button primary label="Save" spinner="changeVehicle({{ $request->id }})"
                        wire:click="changeVehicle({{ $request->id }})" />
                </div>
            </div>
        </x-slot>
    </x-modal.card>
    ...
    <x-modal.card title="Change Driver" align="center" blur wire:model.defer="modifyDriverModal">
        ...
        <x-slot name="footer">
            <div class="flex justify-end gap-x-4">
                <div class="flex">
                    <x-button flat label="Cancel" x-on:click="close" />
                    <x-button primary label="Save" spinner="changeDriver({{ $request->id }})"
                        wire:click="changeDriver({{ $request->id }})" />
                </div>
            </div>
        </x-slot>
    </x-modal.card>
