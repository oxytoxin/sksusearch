<div>
	<div class="grid grid-cols-2 gap-4 text-center md:grid-cols-6">
		<a
			class="flex col-span-2 col-start-1 px-4 py-2 rounded-lg hover:bg-primary-800 hover:text-white bg-primary-600 text-primary-bg-alt active:ring-primary-400 active:ring-4 active:ring-offset-1 lg:row-start-1"
			href="{{ route('requisitioner.travel-orders.create') }}">
			<span class="flex my-auto text-sm break-none">
				<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
					class="h-auto mr-1 w-7 lg:w-5">
		...
		<a
			class="flex col-span-2 col-start-3 px-4 py-2 rounded-lg hover:bg-primary-800 hover:text-white bg-primary-600 text-primary-bg-alt active:ring-primary-400 active:ring-4 active:ring-offset-1 lg:row-start-1"
			href="{{ route('requisitioner.itenerary.create') }}"><span class="flex my-auto text-sm break-none">
				<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
					class="w-5 h-5 mr-1">
					<path stroke-linecap="round" stroke-linejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 11-18 0 9 9 0 0118 0z" />
				</svg>
		...
		@foreach ($categories as $category)
			@if ($category->voucher_types->count() > 1)
				<div class="col-span-6 text-left" x-data="{ openMe: false }">
                    ...
                    x-transition:enter-start =  'opacity-90 scale-0'
			...
			@else
				<a
					class="flex col-span-2 row-start-2 px-4 py-2 rounded-lg hover:bg-primary-800 hover:text-white bg-primary-600 text-primary-bg-alt active:ring-primary-400 active:ring-4 active:ring-offset-1 lg:col-span-1 lg:row-start-1"
