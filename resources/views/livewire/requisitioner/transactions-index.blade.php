        ...
        @foreach ($categories as $category)
            @if ($category->voucher_types->count() > 1)
                <div x-cloak class="col-span-6 text-left" x-data="{ openMe: false }">
                    <h4 x-on:click="openMe=!openMe" class="cursor-pointer hover:text-primary-600">{{ $category->name }}
                    </h4>
                    <div x-show='openMe' x-transition:enter='transition ease-out duration-500' x-transition:enter-start='opacity-90 scale-0' x-transition:enter-end='opacity-100 scale-100' x-transition:leave='transition ease-in duration-300'
                        ...
                        @foreach ($category->voucher_types->sortBy('order_column') as $type)
                            ...
                            @else
                                <div x-data="{ open: false }">
                                    <h5 class="flex items-center px-6 mt-2 hover:cursor-pointer hover:text-primary-600" x-on:click="open=!open">
                                        <span>{{ $type->name }}</span>
                                        <x-ri-arrow-right-s-line x-show="!open" />
                                        <x-ri-arrow-down-s-line x-show="open" />
                                    </h5>
                                    <div x-show='open' x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-100 scale-y-0" x-transition:enter-end=" opacity-100 scale-y-100 "
                                        x-transition:leave="transition ease-in duration-300" x-transition:leave-start=" opacity-100 scale-y-100" x-transition:leave-end=" opacity-0 scale-y-0"
