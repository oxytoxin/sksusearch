					...
					<div id="header" class="items-start block w-full space-y-4 text-left">
						...
						<div class="grid grid-cols-4">
							...
							@if ($travel_order->applicants->count() < 10)
                                ...
                                @endphp
                                @while ($itiretion <= 650)
                                <h4 class="col-span-1 text-sm font-semibold tracking-wide text-black uppercase whitespace-nowrap">{{ $applicant->employee_information->full_name }}-{{ $itiretion }}</h4>
                                @php
                                    $itiretion++;
                                @endphp
                                @endwhile --}}
									...
									@if ($travel_order->applicants->count() > 10)
										<div class="grid grid-cols-3 grid-rows-10">
											@foreach ($travel_order->applicants as $applicant)
												...
												@endphp
												@while ($itiretion <= 650)
													<h4 class="whitespace-nowrap">{{ $applicant->employee_information->full_name }}-{{ $itiretion }}</h4>
													@php
														$itiretion++;
													@endphp
												@endwhile
				...
				<div id="contents" class="flex w-full h-auto px-6 pt-10 print:pt-5">
					<div id="header" class="items-start block w-full space-y-4 text-left">
						<div class="flex-wrap block -space-y-1">
							...
							<span class="block pl-5 font-semibold tracking-wide text-left text-black whitespace-pre-line text-md">
								Your travel is an <span class="underline">{{ $travel_order->travel_order_type->name }}</span>. A report of
