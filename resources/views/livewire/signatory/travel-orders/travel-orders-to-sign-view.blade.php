<div>
					...
					<div class="mt-4 ml-4">
						...
						@foreach ($signatories as $signatory)
							...
							<p class="mt-1 text-sm text-primary-500">Approval Status:
								@switch($signatory->pivot->is_approved)
									@case(0)
										Pending
									@break
							...
							<p class="mt-1 text-sm text-primary-500">Date Approved:
								@switch($signatory->pivot->is_approved)
									...
									@case(1)
										{{ $signatory->pivot->updated_at->format('F d, Y') }}
									@break
							...
							<p class="mt-1 text-sm text-primary-500">Time Approved:
								@switch($signatory->pivot->is_approved)
									...
									@case(1)
										{{ $signatory->pivot->updated_at->format('h:i:s a') }}
									@break
					...
					<div class="flex justify-between w-full">
						...
						<button type="button" wire:click="$set('modal',true)" class="flex px-4 my-auto text-center">
							...
							<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
								stroke="currentColor" class="w-auto h-3 my-auto">
    ...
    <x-modal.card title="Rejection Note" description="Please provide further explanation for rejection of this travel order" blur wire:model.defer="modalRejection" class="text-primary-600">
		<form wire:submit.prevent='reject' class="space=y=2 flex-col">
			<x-textarea class="text-primary-800 placeholder:text-primary-200" label="Your notes" placeholder="Write your notes"
	...
	<x-dialog  id="custom" title="Reject Travel Order"
		description="Confirm Rejection of travel order">
