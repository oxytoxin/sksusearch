<div class="">
            ...
            <div
                ...
                <div class="flex flex-wrap items-center justify-between -mt-4 -ml-4 sm:flex-nowrap">
                    <div class="w-full mt-4 ml-4">
                        <h3 class="text-lg font-medium leading-6 text-primary-900">Request Item Details</h3>
                        ...
                        @php
                            ...
                            $supplyChief = App\Models\EmployeeInformation::where('office_id', 49)
                            ...
                            $finance = App\Models\EmployeeInformation::where('office_id', 25)
                                ...
                                ->first();
                        @endphp
                        <p class="mt-1 text-sm text-primary-500">Unit Cost : &#8369;
                            {{ number_format($record->unit_cost, 2) }}</p>
                        <p class="mt-1 text-sm text-primary-500">PPMP : {{ $record->is_ppmp ? 'Yes' : 'No' }}</p>
                        <p class="mt-1 text-sm text-primary-500">Date Requested :
                            {{ Carbon\Carbon::parse($record->created_at)->format('F d, Y h:i A') }}</p>
                        <p class="mt-4 text-sm text-primary-500 ">Supply Code :
                            @if (
                                ($record->status == 'Pending' ||
                                    $record->status == 'Forwarded to Supply' ||
                                    $record->status == 'Request Modification' ||
                                    $record->status == 'Request Rejected by Supply') &&
                                    !$isSupply &&
                                    $record->supply_code == null)
                                <span class="italic underline ml-2 text-red-600">To be added by supply</span>
                            @elseif(
                                ($record->status == 'Pending' || $record->status == 'Forwarded to Supply') &&
                                    $isSupply &&
                                    $record->supply_code == null)
                                <button class="italic underline ml-2 font-semibold"
                                    wire:click="$set('assignSupplyCode',true)">(Assign Supply Code)</button>
                            @else
                                ...
                                @if ($isSupply && $record->status == 'Accounting Request Modification')
                                    <button class="italic underline ml-2" wire:click="openModifySupplyCodeModal">(Click
                                        to modify)</button>
                                @endif
                            @endif
                        </p>
                        @if (
                            ($record->status == 'Forwarded to Accounting' ||
                                $record->status == 'Pending' ||
                                $record->status == 'Forwarded to Supply' ||
                                $record->status == 'Request Modification' ||
                                $record->status == 'Accounting Request Modification' ||
                                ($record->status = 'Supply Code Assigned')) &&
                                !$isAccountant &&
                                $record->category_item_id == null)
                            <p class="mt-4 text-sm text-primary-500 ">Budget Category : <span
                                    class="italic underline ml-2 text-red-600">To be added by accounting</span></p>
                            <p class="mt-1 text-sm text-primary-500 ">UACS Code : <span
                                    class="italic underline ml-2 text-red-600">To be added by accounting</span></p>
                            <p class="mt-1 text-sm text-primary-500 ">Account Title : <span
                                    class="italic underline ml-2 text-red-600">To be added by accounting</span></p>
                            <p class="mt-1 text-sm text-primary-500 ">Title Group : <span
                                    class="italic underline ml-2 text-red-600">To be added by accounting</span></p>
                        @elseif($record->status == 'Forwarded to Accounting' && $isAccountant && $record->category_item_id == null)
                            <p class="mt-4 text-sm text-primary-500 ">Budget Category : <span
                                    class="italic underline ml-2 font-semibold">Not yet assigned</span>
                            <p class="mt-1 text-sm text-primary-500 ">UACS Code : <span
                                    class="italic underline ml-2 font-semibold">Not yet assigned</span>
                            <p class="mt-1 text-sm text-primary-500 ">Account Title : <span
                                    class="italic underline ml-2 font-semibold">Not yet assigned</span>
                            ...
                            <div class="flex justify-end">
                                <button
                                    class="px-4 py-2 text-sm font-semibold text-white bg-primary-600 rounded-md hover:bg-primary-500"
                                    wire:click="accountingAssign">Assign</button>
                            </div>
                        @elseif($record->status == 'Accounting Assigned Data' && $isAccountant && $record->category_item_id != null)
                            <p class="mt-4 text-sm text-primary-500 ">Budget Category :
                                {{ $record->categoryItems->budgetCategory->name }}</p>
                            <p class="mt-1 text-sm text-primary-500 ">UACS Code :
                                {{ $record->categoryItems->uacs_code }}</p>
                            <p class="mt-1 text-sm text-primary-500 ">Account Title : {{ $record->categoryItems->name }}
                            </p>
            ...
            <div
                ...
                <div class="flex flex-wrap items-center justify-between -mt-4 -ml-4 sm:flex-nowrap">
                    <div class="mt-4 ml-4">
                        ...
                        <p class="mt-4 text-sm text-primary-500">Supply:
                        ...
                        <div class="flow-root">
                            <ul role="list" class="-mb-8">
                                @forelse ($record->wfpRequestTimeline as $timeline)
                                        ...
                                        <li>
                                            <div class="relative pb-8">
                                                ...
                                                <div class="relative flex items-start space-x-3">
                                                    <div class="relative">
                                                        <img class="flex h-8 w-8 items-center justify-center rounded-full bg-gray-400 ring-8 ring-white"
                                                            src="{{ $timeline->user->profile_photo_url }}"
                                                            alt="{{ $timeline->user->name }}">
                                                        </span>
                                                    </div>
                                                    <div class="min-w-0 flex-1">
                                                        ...
                                                        <div class="mt-2 text-sm text-gray-700">
                                                            <p class="italic">To be forwarded to supply for verification
                                                            </p>
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="relative flex items-start space-x-3">
                                                    <div>
                                                        <div class="relative px-1">
                                                            <div
                                                                class="flex h-8 w-8 items-center justify-center rounded-full bg-green-100 ring-8 ring-white">
                                            ...
                                            <div class="relative pb-8">
                                                ...
                                                <div class="relative flex items-start space-x-3">
                                                    ...
                                                    <div class="min-w-0 flex-1">
                                                        ...
                                                        <div class="mt-2 text-sm text-gray-700">
                                                            <p class="italic">Supply code has been assigned and to be
                                                                forwarded to accounting for verification</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div class="relative pb-8">
                                                ...
                                                <div class="relative flex items-start space-x-3">
                                                    <div>
                                                        <div class="relative px-1">
                                                            <div
                                                                class="flex h-8 w-8 items-center justify-center rounded-full bg-yellow-100 ring-8 ring-white">
                                                    ...
                                                    <div class="min-w-0 flex-1">
                                                        <div>
                                                            <div class="text-sm">
                                                                <a href="#"
                                                                    class="font-medium text-gray-900">{{ $timeline->activity }}
                                                                    by
                                            ...
                                            <div class="relative pb-8">
                                                ...
                                                <div class="relative flex items-start space-x-3">
                                                    <div>
                                                        <div class="relative px-1">
                                                            <div
                                                                class="flex h-8 w-8 items-center justify-center rounded-full bg-red-100 ring-8 ring-white">
                                                    ...
                                                    <div class="min-w-0 flex-1">
                                                        <div>
                                                            <div class="text-sm">
                                                                <a href="#"
                                                                    class="font-medium text-gray-900">Request Rejected
                                                                    by
                                                                    {{ $timeline->user->employee_information->full_name }}</a>
                                                            </div>
                                                            <p class="mt-0.5 text-sm text-gray-500">Forwarded:
                                            ...
                                            <div class="relative pb-8">
                                                ...
                                                <div class="relative flex items-start space-x-3">
                                                    ...
                                                    <div class="min-w-0 flex-1">
                                                        ...
                                                        <div class="mt-2 text-sm text-gray-700">
                                                            <p class="italic">UACS Code, Budget Category, Account Title
                                                                and Title Group is assigned by accounting and can be
                                                                added to WFP creation.</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="relative flex items-start space-x-3">
                                                    ...
                                                    <div class="min-w-0 flex-1">
                                                        <div>
                                                            <div class="text-sm">
                                                                <a href="#"
                                                                    class="font-medium text-gray-900">Request
                                                                    Modification by
                                                                    {{ $timeline->user->employee_information->full_name }}</a>
                                                            </div>
                                                            <p class="mt-0.5 text-sm text-gray-500">Forwarded:
                                    ...
                                    @endif
                                @empty
                                    <h3 class="flex justify-between text-sm font-semibold text-primary-800"> Nothing to
                                        show</h3>
                                @endforelse
    ...
    <div class="flex justify-end space-x-3 mt-5 ">
        @if ($record->status == 'Pending' || $record->status == 'Request Modification')
            <a href="{{ route('wfp.request-supply-list') }}"
                class="mr-1 px-3 py-2.5  bg-white rounded-md font-normal capitalize text-primary-600 text-sm">Cancel</a>
            <button wire:click="forwardToSupply"
                class="mr-1 px-3 py-2.5  bg-primary-600 rounded-md font-normal capitalize text-white text-sm">Forward
                to Supply</button>
        @elseif(
            ($record->status == 'Forwarded to Supply' ||
                $record->status == 'Request Rejected by Supply' ||
                $record->status == 'Forwarded to Accounting' ||
                $record->status == 'Accounting Assigned Data' ||
                $record->status == 'Accounting Request Modification' ||
                $record->status == 'Request Rejected by Accounting' ||
                $record->status == 'Supply Code Assigned') &&
                !$isSupply &&
                !$isAccountant)
            <a href="{{ route('wfp.request-supply-list') }}"
        ...
        @elseif($isSupply && $record->supply_code == null)
            <a href="{{ route('wfp.supply-requested-suppluies') }}"
                class="mr-1 px-3 py-2.5  bg-white rounded-md font-normal capitalize text-primary-600 text-sm">Cancel</a>
            <button wire:click="modifyRequest"
                class="mr-1 px-3 py-2.5  bg-yellow-600 rounded-md font-normal capitalize text-white text-sm">Modify
                Request</button>
            <button wire:click="rejectRequest"
                class="mr-1 px-3 py-2.5  bg-red-600 rounded-md font-normal capitalize text-white text-sm">Reject
                Request</button>
        @elseif(
            ($record->status == 'Forwarded to Supply' || $record->status == 'Supply Code Assigned') &&
                $isSupply &&
                $record->supply_code != null)
            <a href="{{ route('wfp.supply-requested-suppluies') }}"
                class="mr-1 px-3 py-2.5  bg-white rounded-md font-normal capitalize text-primary-600 text-sm">Cancel</a>
            <button wire:click="forwardToAccounting"
                class="mr-1 px-3 py-2.5  bg-primary-600 rounded-md font-normal capitalize text-white text-sm">Forward
                to Accounting</button>
        @elseif(
            ($record->status == 'Forwarded to Accounting' || $record->status == 'Accounting Request Modification') &&
                $isSupply &&
                $record->supply_code != null)
            <a href="{{ route('wfp.supply-requested-suppluies') }}"
                class="mr-1 px-3 py-2.5  bg-white rounded-md font-normal capitalize text-primary-600 text-sm">Cancel</a>
        @elseif($record->status == 'Forwarded to Accounting' && $isAccountant)
            <button wire:click="$set('forwardRequestToSupply', true)"
                class="mr-1 px-3 py-2.5  bg-yellow-600 rounded-md font-normal capitalize text-white text-sm">Forward
                Request to Supply</button>
            <button wire:click="rejectRequestAccountingModal"
                class="mr-1 px-3 py-2.5  bg-red-600 rounded-md font-normal capitalize text-white text-sm">Reject
                Request</button>
        @elseif($record->status == 'Accounting Assigned Data' && $isAccountant)
            <a href="{{ route('wfp.accounting-requested-suppluies') }}"
                class="mr-1 px-3 py-2.5  bg-white rounded-md font-normal capitalize text-primary-600 text-sm">Cancel</a>
        @endif
    </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-1 mb-1">
            <div class="col-span-1">
                <div class="flex justify-between">
                    ...
                    <label for="uacs_code" class="block text-xs font-medium leading-6 text-red-900">*Automatically
                        filled when selecting account title</label>
                </div>
                <div class="mt-2">
                    <div
