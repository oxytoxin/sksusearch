...
<body class="p-8 text-gray-900">
    ...
    <div class="text-center my-4">
        <h2 class="text-xl font-bold uppercase tracking-wide">Transmittal</h2>
        <p class="text-sm">No. {{ $transmittal->transmittal_number }}</p>
    </div>

    <div class="flex justify-between text-sm mb-4">
        <div><span class="font-semibold">Transmitted To:</span> {{ $transmittal->recipient }}</div>
        <div><span class="font-semibold">Date:</span> {{ optional($transmittal->created_at)->format('F d, Y') }}</div>
    </div>
    ...
    @if ($transmittal->remarks)
        <p class="mt-3 text-sm"><span class="font-semibold">Remarks:</span> {{ $transmittal->remarks }}</p>
    @endif
    ...
    <div class="grid grid-cols-2 gap-8 mt-16 text-sm">
        ...
        <div>
            ...
            <p class="text-xs text-gray-500">Received By (Signature over Printed Name &amp; Date)</p>
        </div>
    </div>
